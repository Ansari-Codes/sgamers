from UI import Label, Button, Icon, RawCol, AddSpace, Header, Notify, ui
...
async def changePage(area:ui.element, var:Variable, name:str, triggerer=None, user_id=None,name_=None):
    ...
    with area:
        ...
        try:
            if name == 'dashboard': await dashboard(area, user_id, name_) # type:ignore
            elif name == 'games': await games(area,user_id)
            elif name == 'settings': await settings(area)
        except Exception as e:
            Notify(f"We cannot take you to {name}! {e}", type="negative")
        finally:
            if triggerer:
    ...
    with ui.drawer('left') as drawer:
        ...
        with RawCol().classes("w-full h-full gap-1"):
            ...
            for name, kw in btns.items():
                b = Button(name, config=kw).classes("w-full").props('align="left"')
                bs.append((lambda b=b: b)())
                b.on_click(lambda _,name=name,b=bs: changePage(area, var, name, b,user_id,uname))
            sb = Button("Settings", config=dict(icon="settings")).classes("w-full").props('align="left"')
            bs.append(sb)
            sb.on_click(lambda bs=bs:changePage(area, var, "settings",bs,user_id,uname))
            db = Button("Docs", config=dict(icon="book")).classes("w-full").props('align="left"')
            bs.append(db)
